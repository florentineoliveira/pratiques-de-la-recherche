...
`ggplot2` est un package `R` qui permet de **visualiser des données** sous forme de graphiques. Afin de chaîner des opérations ensemble, c'est à dire d'**ajouter des couches au graphique, on utilise les `+`**
...
2. Lier le jeu de données qui contient l'information à visualiser avec les données géographiques  
  - Pour ajouter de l'information à un fond de carte, il est nécessaire de disposer d'une **variable (*key*) géographique commune entre les deux fichiers**:
    - Par exemple si vous souhaitez représenter la carte des réusltats aux éléctions législatives de 2024 en France à l'échelle départementale, il est nécessaire que le fichier qui recence les résultats et le fichier qui recence les données géographiques aient une variable commune qui permet de les lier: nom de département, numéro de département
