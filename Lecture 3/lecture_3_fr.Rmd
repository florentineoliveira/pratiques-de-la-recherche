---
title: "Lecture 3: Régressions linéaires"
...
---
# Programme de cette lecture 

## Notions

**Régression linéaire** et **estimateur des MCO**

### Application

**Taille de la fratrie** et **performances scolaires** des enfants.
  - Données simulées à partir de la description de l'échantillon faite dans l'article


---
# Régression linéaire simple

On s'intéresse à la relation entre deux variables:

--

  * une __variable expliquée__ (ou __variable dépendante__ ou ***outcome***), *** $y$ ***
  
--
  
  * an __variable explicative__ (ou __variable indépendante__ ou __régresseur__), *** $x$ ***
...
* Pour le moment, cette relation est synthétisée par une droite, ayant pour constante $\alpha$ et pente $\beta$:
...
* ** $\hat{y}_i$ ** est la **prediction** de $y$ pour l'observation $i$ $(y_i)$.
...
```{R, nuage, echo = F, dev = "svg", fig.width = 12, fig.height = 5}
...
Pour toute droite $\hat{y} = \hat{\alpha} + \hat{\beta} x$

```{R, nuage et droite, echo = F, dev = "svg", fig.width = 12, fig.height = 5}
...
ggplot(data = df, aes(x = x, y = y)) +
    ...
    annotate("text", x = -0.4, y = 3.2, label = "hat(y)", parse = TRUE, color = "#f08b27", size = 6, family = "serif") +
    scale_x_continuous(limits = c(-1,8)) +
    scale_y_continuous(limits = c(0.5,7)) +
    theme_empty
...
Pour toute droite $\hat{y} = \hat{\alpha} + \hat{\beta} x$, on peut calculer les erreurs: $e_i = y_i - \hat{y}_i$

```{R, nuage et droite et point y2, echo = F, dev = "svg",fig.width = 12, fig.height = 5}
...
```{R, nuage et droite et point y2 et e2, echo = F, dev = "svg", fig.width = 12, fig.height = 5}
...
ggplot(data = df, aes(x = x, y = y)) +
    ...
    annotate("text", x = -0.4, y = 3.2, label = "hat(y)", parse = TRUE, color = "#f08b27", size = 6, family = "Palatino") +
...
```{R, nuage et droite et toutes erreurs, echo = F, dev = "svg", fig.width = 12, fig.height = 5}
...
```{R, nuage et autre droite et toutes erreurs , echo = F, dev = "svg", fig.width = 12, fig.height = 5}
...
ggplot(data = df, aes(x = x, y = y)) +
    ...
    geom_abline(intercept = b0, slope = b1, color = "#f08b27", size = 1, alpha = 0.9) +
    geom_segment(aes(x = x, xend = x, y = y, yend = y_hat(x, b0, b1)), size = 0.5, alpha = 0.2) +
    annotate("text", x = -0.4, y = 5.6, label = "hat(y)", parse = TRUE, color = "#f08b27", size = 6, family = "Palatino") +
    scale_x_continuous(limits = c(-1,8)) +
    scale_y_continuous(limits = c(0.5,7)) +
    theme_empty
```
...
La SCE somme les erreurs au carré $\left(\sum \varepsilon_i^2\right)$: les erreurs importantes ont de plus grosses pénalités.

```{R, penalties, echo = F, dev = "svg", fig.width = 12, fig.height = 5}
...
L'estimateur des MCO (OLS) calcule $\hat{\alpha}$ and $\hat{\beta}$ qui minimisent la SCE.

```{R, mco penalties, echo = F, dev = "svg", fig.width = 12, fig.height = 5}
# Define a function
y_hat <- function(x, b0, b1) {b0 + b1 * x}
# Define line's parameters
...
```{R, mco, echo = F, dev = "svg",fig.width = 12, fig.height = 5}
...
```{R, scatter1 opur sample 2, echo = F, fig.fullwidth = T, dev = "svg"}
ggplot(data = df, aes(x = x, y = y)) +
geom_abline(
  intercept = lm0$coefficients[1], slope = lm0$coefficients[2],
  color = "#dd0747", size = 2) +
...
```{R, scatter1 opur sample 3, echo = F, fig.fullwidth = T, dev = "svg"}
ggplot(data = df, aes(x = x, y = y)) +
geom_abline(
  intercept = lm0$coefficients[1], slope = lm0$coefficients[2],
  color = "#dd0747", size = 2) +
...
.pull-left[
```{R, simulation, echo = F, dev = "png", dpi = 300, cache = T}
...
.pull-left[
```{R, selected sample, echo = F, dev = "png", dpi = 300, cache = T}
...
class:: inverse
## Application : taille de la fratrie et performances scolaires
...
```{r, echo=FALSE, include = F, fig.width = 12, fig.height = 5}
...
---
## Hypothèses de l'estimateur des MCO

- $H_1$ **Linéarité**: le modèle est linéaire dans les paramètres. Formellement, $\frac{\partial y_i}{\partial x_{ik}} = \beta_k$ $\forall k =1, ...; K$
...
- $H_3$ : Il y a suffisamment de variation dans X. Formellement, s'il y a plusieurs variables explicatives, elles ne sont pas colinéaires, donc $(X'X)$ est inversible. 

--

- $H_4$ : epsilon est iid et de distribution N(0, sigma 2) homoscédasticité
